...
############################################################# VV ###############################################################################
    ...
    if Rover.total_time > 10:
        ...
        wall_offset = 0.8 * np.std(Rover.nav_angles)  ## it used 0.8 
######################################################### ^^ ##################################################################################
...
####################################################################### VV ################################################################
            if Rover.rocks_angles is not None and np.mean(Rover.rocks_angles) > -0.2 and np.min(Rover.rocks_dists) < 30:
                # Rover.steer = np.clip(np.mean(Rover.rocks_angles * 180 / np.pi), -15, 15)
                Rover.time_rock = Rover.total_time
                Rover.mode.append('collect')
############################################################################# ^^ ##########################################################
...
############################################################################ VV ###########################################################
                if Rover.vel <= 0.1 and Rover.total_time - Rover.time_stuck > 4:
                    # Set mode to "stuck" and hit the brakes!
                    Rover.throttle = 0
                    # Set brake to stored brake value
                    Rover.brake = Rover.brake_set
                    Rover.steer = 0
                    Rover.mode.append('crash')
                    Rover.time_stuck = Rover.total_time
...
######################################################################### ^^ #########################################################
                ...
                Rover.steer = np.clip(np.mean((Rover.nav_angles + wall_offset) * 180 / np.pi), -15, 15) ################## difference detected 
...
############################################################ VV #####################################################################
        ...
        elif Rover.mode[-1] == 'crash':
            # if 1 sec passed go back to previous mode
            if Rover.total_time - Rover.time_stuck > 1:
                ...
                # Hug left wall by setting the steer angle slightly to the left
                Rover.steer = np.clip(np.mean((Rover.nav_angles + wall_offset) * 180 / np.pi), -15, 15)
        ...
        elif Rover.mode[-1] == 'collect':
            # Steer torwards the sample
            mean = np.mean(Rover.rocks_angles * 180 / np.pi)
            ...
            if Rover.total_time - Rover.time_rock > 30:
                Rover.mode.pop()  # returns to previous mode
            ...
            elif Rover.vel <= 0 and Rover.total_time - Rover.time_stuck > 10:
                Rover.throttle = 0
                # Set brake to stored brake value
                Rover.brake = Rover.brake_set
                Rover.steer = 0
        ...
        # If we're already in "stop" mode then make different decisions
            ...
            # If we're not moving (vel < 0.2) then do something else
                ...
                if len(Rover.nav_angles) >= Rover.go_forward:
                    ...
                    # Set steer to mean angle
                    # Hug left wall by setting the steer angle slightly to the left
                    wall_offset = 12 #  #                                                       ########################### difference detected 
                    Rover.steer = np.clip(np.mean(Rover.nav_angles * 180 / np.pi) + wall_offset, -15, 15) ################# difference detected 
                    Rover.mode.pop()  # returns to previous mode
